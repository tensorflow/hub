...
def text_module_fn():
  ...
  text = tf.compat.v1.placeholder(tf.string, shape=[None])
  hash_buckets = tf.compat.v1.string_to_hash_bucket_fast(
      text, weights.get_shape()[0])
  embeddings = tf.compat.v1.gather(weights, hash_buckets)
